...
**R18 fix (landed, verified on the arm1 rig).** `src/fan_lua_lock.h` overrides `luai_threadyield` in the hooked build so that it re-derives the frame base after re-locking — `{lua_unlock(L); lua_lock(L); base = ci->u.l.base;}` on 5.3, `base = ci->func.p + 1` on 5.4 (which dropped that field, and whose `Protect` is `(savestate(L,ci),(exp),updatetrap(ci))`) — and `tests/build_hooked_lua.sh` carries the version switch that selects it (5.4.x → `-DFAN_LUA_STACK_BASE_54=1`). Four steps on the rig (`docker luafan-cmake-a`, Linux/arm64; the tree is byte-identical to the commit — all 16 files checked with `md5sum` matched): (1) the landed build script, run end to end from a clean tarball, installs a 5.3.6 interpreter that exports `LockMainState`/`UnLockMainState`/`LuaCoreLockHooked`, and `gcc -E` on `lvm.c` shows all 3 `checkGC` sites expanding to `{ UnLockMainState(L); LockMainState(L); base = (ci)->u.l.base; }`; (2) that interpreter passes `tests/lua/test_lock_granularity.lua` (`core-hook`, all 11 cases, including `core-hook: a busy Lua worker leaves the main thread runnable (max gap < 0.15s)`) — the unlock is still handed over; (3) the whole runner with it (`LUAN_TEST_WORKERS=4 LUAN_LOCK_MODE_EXPECT=core-hook LUAFAN_LUA_BIN=<landed interpreter> bash tests/run_lua_tests.sh`) is green — curated **43 passed / 1 skipped**, lock granularity, all 14 standalone suites, `rc=0` — and `run_c_tests` reports **220/220 PASSED** in `build`, `build-normal` and `build-asan`; (4) the fix itself, A/B against a same-tarball control in which the A/B is only the landed header (ASan armed in both arms): the standalone reproducer goes from **10/10 crash to 0/10** on both 5.3.3 and 5.4.6, and `test_async_httpd.lua` (the natural-load WebSocket case) from **6/40** ASan reports to **0/40**, while `-DFAN_LUA_THREADYIELD_FIX=0` on the same landed tree brings the reports back (**2/40**, `luaD_precall ldo.c:416` ← `luaV_execute lvm.c:1134`, freed by `luaD_reallocstack`; baseline on the unfixed build: `luaD_precall ldo.c:347` ← `luaV_execute lvm.c:1134` ← `_utlua_resume` ← `httpd_handler_cgi_bin`, freed by `luaD_reallocstack` ← `traversethread` ← `lua_gc`) — i.e. both the switch and the `#if !defined(luai_threadyield)` guard work. The window stays real on the fixed build: the standalone runs still open it ~2036 (5.3.3) / ~2044 (5.4.6) times and the collector still shrinks the running victim (11264 → 34 or 49 slots on 5.3.3, 10240 → 56 on 5.4.6), with the victim completing (`calls=2001`, identical `sum`) — the fix removes the stale pointer, not the race. Probes (site workspace `tmp/`, uploaded to the rig under `/tmp/`): `r18_standalone.c`, `r18_standalone_lock.h`, `r18_landed_build.sh`, `r18_landed_standalone.sh`, `r18_ws_sweep.sh`. **Not covered:** the Apple hook is a different header (`luan_mac/lua-apple/luauser.h`, reached through `-DLUA_USER_H`, which routes `lua_lock`/`lua_unlock` the same way), so an Apple build still carries R18 and needs the same override mirrored there.

**R22 follow-up (review fix, landed).** Two statements in the R22 row above describe the first landed shape of that fix and are superseded here. (1) *"up to 250 in total ... falls through to the old behaviour ... the claim is always released"*: the deferral is bounded again (250 tries, ~1.05 s) and a coroutine that never suspends is now **dropped** by `wait_defer_abandon()` — claim released, `bag` and its event freed, coroutine left suspended — instead of being resumed, because neither `LUA_OK` (the arming C call may still own the coroutine) nor a dead coroutine can be resumed safely. The same helper handles a retry timer that cannot be re-armed, so no path returns with the claim still held: without it, that branch left `pending->in_flight` raised for good, which blocked every later close in `wait_for_foreign_continuations()`, and the unbounded 5 ms retry leaked the timer, the bag and the claim for the same reason. (2) The close drain no longer replays a foreign continuation on the closing thread: each wait records `owner_worker_id` and `mariadb_cancel_pending_waits()` hands the cancellation back to that base (`event_mgr_worker_once_internal()`, which the owner base's own `event_mgr_drain_internal_jobs()` also runs during shutdown), and it replays locally only when no owner thread can run the job (`!event_mgr_is_loop_running()` — the phase Lua `__gc` finalisers run in, where the bases are drained and joined — or a hand-off that cannot be allocated). No path re-arms a wait any more: the earlier "re-arm it and break" branch would have let it fire after `mysql_close*()` released the connection, i.e. exactly the R19/R20 use-after-free, and it also abandoned the drain with the wait list non-empty. `luamariadb_connect.c` lost its last post-connect `MYSQL_OPT_RECONNECT` call as well (the option is set and error-checked before the connect starts, so the synchronous-completion path had nothing left to apply). Verification (arm1, `docker luafan-cmake-a`, `LUAN_TEST_WORKERS=4`): `run_c_tests` **220/220** in `build` (core-hook), `build-normal` (wrapper) and `build-asan`, with **0** compiler warnings on the recompiled translation units; `test_mariadb_pending_event.lua` 11/11 runs on core-hook and 3/3 on wrapper, `test_mariadb_workers.lua` 11/11 and 3/3; ASan (`build-asan` + the ASan hooked 5.3.6 interpreter) **0 reports** across `test_mariadb_pending_event.lua` (3 runs), `test_mariadb_workers.lua` (6) and the new `test_mariadb_pending_owner.lua` (3); the full `run_lua_tests.sh` runner is green (curated 43 pass / 1 skip + lock granularity + all 15 standalone suites, `rc=0`). New guard: `tests/lua/test_mariadb_pending_owner.lua` parks a `SELECT SLEEP(3)` on **worker 1's** base and closes the connection from the main thread (the hand-off path, three rounds, so a leaked claim would hang the next close), then repeats the same close on the main base (the inline `event_mgr_is_current_owner()` path) and finally checks that a fresh connection still serves a query; the pre-existing `test_mariadb_pending_event.lua` only ever closed from the thread that armed the wait. The two remaining review items are documented rather than fixed: the drain's hand-off needs one heap allocation (a failed one falls back to the pre-owner inline replay, which races the owner loop in that OOM-only window), and a wait queued to a base that has *already* finished its shutdown drain would never run (it is unreachable while a parked wait keeps its coroutine, and hence the connection, referenced).
