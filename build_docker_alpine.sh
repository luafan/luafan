...
apk add --update bsd-compat-headers tzdata linux-headers git lua5.3-dev lua5.3 libstdc++ wget ca-certificates gcc libc-dev unzip cmake g++ make libevent libevent-dev curl-dev curl ncurses-dev bison openssl-dev openssl perl sqlite-dev \
    ...
    && git clone --depth 1 --branch 0.4.1.53-luafan1 https://github.com/luafan/lzlib.git /tmp/lzlib-luafan && cd /tmp/lzlib-luafan && luarocks make rockspec/lzlib-0.4.1.53-luafan1.rockspec ZLIB_LIBDIR=/lib && cd - && rm -rf /tmp/lzlib-luafan && luarocks install openssl && luarocks install lbase64 \
