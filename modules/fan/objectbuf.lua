...
packer["object"] = function(ctx, obj)
    ...
    if p then
        ...
        error("unsupported pack type", type(obj), obj)
    end
end
    ...
    if has_table then
        ...
        for i=1,count do
            ...
            while d:available() > 0 do
                local k = index_map[d:GetU30()]
                local v = index_map[d:GetU30()]
                t[k] = v
            end
        end
    end

    return index_map[last_top] -- refer to the first of table item
end
...
local function encode(obj)
    ...
    if #(ctx.integer_u30s) > 0 then
        flag = flag + HAS_U30_MASK
        bodystream:AddU30(#(ctx.integer_u30s))
    ...
    if #(ctx.strings) > 0 then
        flag = flag + HAS_STRING_MASK
        bodystream:AddU30(#(ctx.strings))
        for i,v in ipairs(ctx.strings) do
            bodystream:AddString(v)
            index_map[v] = index + i
        end
    end
    ...
    if (#ctx.tables) > 0 then
        ...
        for i,tb in ipairs(ctx.tables) do
            local d = stream.new()
            for k,v in pairs(tb) do
                d:AddU30(index_map[k])
                d:AddU30(index_map[v])
            end
