...
local function prepare(ctx, db, sql)
	local stmt = ctx[PREPARE_STMT_CACHE_KEY][sql]
	if config.debug then
		print("prepare", sql, stmt and "with cache")
	end
	if not stmt then
		stmt = assert(db:prepare(sql))
		ctx[PREPARE_STMT_CACHE_KEY][sql] = stmt
	end
