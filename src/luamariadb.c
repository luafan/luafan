...
static void free_result_cont(int fd, short event, void *_userdata) {
  ...
  if (status) {
    wait_for_status(L, ms->conn_data, cur->my_res, status, free_result_cont, ms->extra);
  } else {
...
static void stmt_store_result_cont(int fd, short event, void *_userdata) {
  ...
  if (errorcode) {
    ...
    wait_for_status(L, st->conn_data, st, status, stmt_store_result_cont, ms->extra);
  } else if (ret == 0) {
...
static void stmt_prepare_cont(int fd, short event, void *_userdata) {
  ...
  if (errorcode) {
    ...
    if (status) {
      wait_for_status(L, ms->conn_data, st, status, stmt_prepare_cont, ms->extra);
      skip_unref = 1;
    } else if (ret == 0) {
...
static void conn_commit_event(int fd, short event, void *_userdata) {
  ...
  if (errorcode) {
    ...
    wait_for_status(L, ms->conn_data, conn, status, conn_commit_event, ms->extra);
  } else if (ret == 0) {
    lua_pushboolean(L, true);
    utlua_resume(L, NULL, 1);
  } else {
...
static void conn_rollback_event(int fd, short event, void *_userdata) {
  ...
  if (errorcode) {
    ...
    wait_for_status(L, ms->conn_data, conn, status, conn_rollback_event, ms->extra);
  } else if (ret == 0) {
    lua_pushboolean(L, true);
    utlua_resume(L, NULL, 1);
  } else {
...
static void conn_autocommit_event(int fd, short event, void *_userdata) {
  ...
  if (errorcode) {
    ...
    wait_for_status(L, ms->conn_data, conn, status, conn_autocommit_event, ms->extra);
  } else if (ret == 0) {
    lua_pushboolean(L, true);
    utlua_resume(L, NULL, 1);
  } else {
...
static void set_character_set_cont(int fd, short event, void *_userdata) {
  ...
  if (status) {
    wait_for_status(L, ms->conn_data, conn, status, set_character_set_cont, ms->extra);
  } else if (ret == 0) {
    lua_pushboolean(L, 1);
    utlua_resume(L, NULL, 1);
  } else {
...
static void real_connect_cont(int fd, short event, void *_userdata) {
  ...
  if (status) {
    wait_for_status(L, ms->conn_data, conn, status, real_connect_cont, ms->extra);
    skip_unref = 1;
  } else if (ret == conn) {
    char value = 1;
