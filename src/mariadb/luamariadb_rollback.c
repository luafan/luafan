#include "luamariadb_rollback.h"

static void conn_rollback_event(int fd, short event, void *_userdata)
{
  DB_STATUS *bag = (DB_STATUS *)_userdata;
  MYSQL *conn = (MYSQL *)bag->data;
  lua_State *L = bag->L;

  int errorcode = mysql_errno(conn);
  if (errorcode)
  {
    int nresults = luamariadb_push_errno(L, bag->ctx);
    UNREF_CO(bag->ctx);
    FAN_RESUME(L, NULL, nresults);
  }
  else
  {
    my_bool ret = 0;
    int status = mysql_rollback_cont(&ret, conn, bag->status);
    if (status)
    {
      if (wait_for_status(L, bag->ctx, conn, status, conn_rollback_event,
                          bag->extra) != 0)
      {
        int nresults = mariadb_push_wait_error(L);
        UNREF_CO(bag->ctx);
        FAN_RESUME(L, NULL, nresults);
      }
    }
    else if (ret == 0)
    {
      lua_pushboolean(L, true);
      UNREF_CO(bag->ctx);
      FAN_RESUME(L, NULL, 1);
    }
    else
    {
      int nresults = luamariadb_push_errno(L, bag->ctx);
      UNREF_CO(bag->ctx);
      FAN_RESUME(L, NULL, nresults);
    }
  }
  event_free(bag->event);
  free(bag);
}

LUA_API int conn_rollback_start(lua_State *L)
{
  DB_CTX *ctx = getconnection(L);
  my_bool ret = 0;
  int status = mysql_rollback_start(&ret, &ctx->my_conn);
  if (status)
  {
    REF_CO(ctx);
    if (wait_for_status(L, ctx, &ctx->my_conn, status, conn_rollback_event, 0) != 0)
    {
      UNREF_CO(ctx);
      return mariadb_push_wait_error(L);
    }
    return lua_yield(L, 0);
  }
  else if (ret == 0)
  {
    lua_pushboolean(L, true);
    return 1;
  }
  else
  {
    return luamariadb_push_errno(L, ctx);
  }
}
