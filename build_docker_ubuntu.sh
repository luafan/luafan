...
apt update \
    ...
    && cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DCMAKE_C_FLAGS="-Wno-unknown-warning-option -Wno-stringop-overflow -Wno-array-bounds" -GNinja .. \
    && ninja \
    && mkdir -p lib && ln -sf ../crypto/libcrypto.a lib/libcrypto.a && ln -sf ../ssl/libssl.a lib/libssl.a && cp -Rf ../include . \
    && cd ../.. \
    && cp -r boringssl/include/openssl /usr/local/include/ \
    && cp boringssl/build/ssl/libssl.a /usr/local/lib/ && cp boringssl/build/crypto/libcrypto.a /usr/local/lib/ \
    ...
    && make -j$(nproc) && make install && ldconfig && cd ../.. && rm -rf curl-impersonate \
    ...
    && apt-get -y remove g++ bison libncurses5-dev liblua5.3-dev libc-dev zlib1g-dev unzip cmake make gcc binutils libc-dev-bin git golang-go ninja-build autoconf automake libtool pkg-config \
