...
cd && apt update \
    ...
    && luarocks make luafan-$LUAFAN_VERSION.rockspec MARIADB_VERSION_DIR=/usr/local/mysql CURL_INCDIR=/usr/include/`uname -m`-linux-gnu \
    ...
    && cd && rm -rf openssl* /usr/local/share/doc /usr/local/mysql/lib/*.a /var/lib/apt/lists/* || exit -1
